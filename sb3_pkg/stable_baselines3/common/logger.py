...
def register_output_format(format: str, formatter: KVWriter, formatter_kwargs):
    """
    register a custom formatter for logging
    """
    if not format in format_classes:
        format_classes[format] = (formatter, formatter_kwargs)
...
def make_output_format(_format: str, log_dir: str, log_suffix: str = "") -> KVWriter:
    ...
    elif _format in format_classes:
        formatter, formatter_kwargs = format_classes[_format]
        return formatter(log_dir, **formatter_kwargs)
    else:
