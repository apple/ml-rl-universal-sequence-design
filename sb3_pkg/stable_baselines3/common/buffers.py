...
class RolloutBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        super().__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
        self.graph_obs_space = isinstance(self.observation_space, GenericGraphSpace)
    ...
    def to_graph_batch(self, array):
        return Batch.from_data_list(array.flatten().tolist()).to(self.device)
    ...
    def _get_samples(
        ...
        array_type_vec = np.array([
            not self.graph_obs_space,
            True,
            True,
            True,
            True,
        ...
        graph_type_vec = np.logical_not(array_type_vec)
        data = [self.to_graph_batch(elem) if graph_type else elem for elem, graph_type in zip(data, graph_type_vec)]
        data = [self.to_torch(elem) if array_type else elem for elem, array_type in zip(data, array_type_vec)]
