...
class CheckpointCallback(BaseCallback):
    ...
    def _checkpoint_path(self, checkpoint_type: str = "", extension: str = "") -> str:
        ...
        """
        return os.path.join(self.save_path, f"{self.name_prefix}_{checkpoint_type}{self.num_timesteps * self.world_size}_steps.{extension}")

    def _on_step(self) -> bool:

        if self.use_ddp:
            # only master process does checkpointing
...
#            model_path = self._checkpoint_path(extension="zip")
            ...
            print(f"Saving model checkpoint to {self.checkpoint_path}")
#            if self.verbose >= 2:
